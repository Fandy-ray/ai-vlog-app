...
interface BindAccountSheetProps {
  open: boolean
  onClose: () => void
  onSuccess?: () => void
}

export function BindAccountSheet({ open, onClose, onSuccess }: BindAccountSheetProps) {
  ...
  const {
    ...
    verifyCode,
  } = useVerificationCode('bind-account')
  ...
  useEffect(() => {
    ...
    setError('')
    resetVerificationCode()
  }, [open, resetVerificationCode, user?.phone])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { needsPasswordSetup } = login(phone, user?.nickname, {
    ...
    >
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 id="bind-account-title" className="text-lg font-semibold text-text">
              绑定账号
            </h2>
            <p className="mt-1 text-xs text-text-muted">验证手机号后即可完成绑定或换绑</p>
          </div>
          <button
            type="button"
            onClick={onClose}
