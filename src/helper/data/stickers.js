...
export function createDefaultStickerOverlay(stickerId, index = 0, videoAspect = 16 / 9) {
  const offset = index % 5
  const size = 18
  const width = videoAspect >= 1 ? size / videoAspect : size
  const height = videoAspect >= 1 ? size : size * videoAspect
  return {
    id: createStickerId(),
    stickerId,
    x: 50 + (offset % 3) * 10,
