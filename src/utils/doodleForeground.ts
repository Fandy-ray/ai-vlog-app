import {
  ...
  normalizeStickerDimensions,
  squareStickerDimensions,
} from '@/data/stickers'
...
export async function resolveDoodlePlacement(
  ...
  const height = clamp(((bottom - top) / canvas.height) * 100, 18, 50)
  const { width } = normalizeStickerDimensions(0, height, contentAspect, videoAspect)

  return {
    x: ((left + right) / 2 / canvas.width) * 100,
    y: ((top + bottom) / 2 / canvas.height) * 100,
    width: clamp(width, 18, 50),
    height,
  }
}
